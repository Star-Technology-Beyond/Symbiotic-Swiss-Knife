...
public class MultitoolRadialScreen extends Screen {
    ...
    private MultitoolMode hoveredMode;
    private MultitoolMode selectedMode;
    private List<MultitoolMode> installedModes;

    // stacks used for button icons, we need to lazily load these
    private ItemStack barrierStack;
    private ItemStack steelGearStack;

    protected MultitoolRadialScreen(ItemStack stack, InteractionHand hand) {
        super(Component.translatable("key.symbiotic_swiss_knife.multitool_selector"));
        this.stack = stack;
        this.hand = hand;
        this.installedModes = MultitoolMode.getInstalled(stack);
        this.selectedMode = MultitoolMode.getActive(stack);
    }
    ...
    @Override
    public void tick() {
        if (MultitoolClientEvents.getHeldMultitool() == null) {
            onClose();
            return;
        }
        installedModes = MultitoolMode.getInstalled(stack);
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        boolean singleBlock = MultitoolMode.isSingleBlockMode(stack);
        MultitoolRadialButton singleBlockButton = singleBlockButton(singleBlock);
        MultitoolRadialButton configButton = configButton();
        ...
        for (MultitoolMode mode : installedModes) {
        ...
        MultitoolMode labelMode = hoveredMode == null ? selectedMode : hoveredMode;
        if (labelMode != null) {
            ...
            if (isSneaking && hoveredMode != null) {
                graphics.drawCenteredString(font,
                        Component.translatable("key.symbiotic_swiss_knife.multitool_selector.eject")
                                .withStyle(ChatFormatting.RED),
                        centerX, centerY + 4, COL_HINT_EJECT);
            } else {
                graphics.drawCenteredString(font,
                        Component.translatable("key.symbiotic_swiss_knife.multitool_selector.hint"),
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (singleBlockButton(singleBlock).isHovered(mouseX, mouseY)) {
            SymbioticNetwork.NETWORK.sendToServer(new CPacketToggleSingleBlockMode(hand));
            MultitoolMode.toggleSingleBlockMode(stack);
            return true;
        }

        // config button to show the auto select screen
        if (configButton().isHovered(mouseX, mouseY)) {
            minecraft.setScreen(new MultitoolAutoSelectScreen(stack, hand));
            return true;
        }

        // fall through to circle
        MultitoolMode mode = getMouseMode(width / 2, height / 2, mouseX, mouseY);
        if (mode != null) {
            if (hasShiftDown()) {
                eject(mode);
            } else {
    ...
    private MultitoolRadialButton singleBlockButton(boolean singleBlock) {
        return new MultitoolRadialButton(
                singleBlockButtonX(), buttonY(), BUTTON_WIDTH, BUTTON_HEIGHT,
                barrierStack,
                Component.translatable(singleBlock
                        ? "item.symbiotic_swiss_knife.gregtech_multitool.single_block_on"
                        : "item.symbiotic_swiss_knife.gregtech_multitool.single_block_off"));
    }

    private MultitoolRadialButton configButton() {
        return new MultitoolRadialButton(
                autoSelectConfigButtonX(), buttonY(), BUTTON_WIDTH, BUTTON_HEIGHT,
                steelGearStack,
                Component.translatable("item.symbiotic_swiss_knife.gregtech_multitool.auto_select_btn"));
    }

    private void select(MultitoolMode mode) {
        selectedMode = mode;
        MultitoolClientEvents.selectMode(stack, hand, mode);
    }

    private void eject(MultitoolMode mode) {
        MultitoolClientEvents.ejectMode(stack, hand, mode);
    }

    // calculate what mode we're on at the mouse position
    private MultitoolMode getMouseMode(int centerX, int centerY, double mouseX, double mouseY) {
        if (installedModes.isEmpty())
            return null;
    ...
    private void drawModeCircleSegments(Matrix4f matrix, int centerX, int centerY) {
        ...
        // we dont want to break the rest of gui rendering so restore all the settings
        // we changed
    ...
    private double centerAngle(MultitoolMode mode) {
    ...
    private ItemStack getRepresentativeStack(MultitoolMode mode) {
        try {
